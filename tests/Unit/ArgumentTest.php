...
class ArgumentTest extends TestCase
{
    ...
    public function test_argument_is_optional_return_true_if_argument_is_marked_as_optional()
    {
        $arg = new Argument(DataTypes::ANY, true);
        $this->assertTrue($arg->isOptional());
    }

    public function test_argument_get_type_return_provided_type_value()
    {
        $arg = new Argument('string', true);
        $this->assertEquals('string', $arg->getType());
    }


    public function test_argument_match_return_false_if_scalar_type_does_not_match()
    {
        $arg = new Argument('int', false);
        $this->assertFalse($arg->match('Hello World'));
        $arg = new Argument('string', false);
        $this->assertFalse($arg->match(4.5));
    }

    
    public function test_argument_match_return_true_if_scalar_type_does_match()
    {
        $arg = new Argument('string', false);
        $this->assertTrue($arg->match('Hello World'));
        $arg = new Argument('double', false);
        $this->assertTrue($arg->match(4.5));
    }

    public function test_argument_match_return_false_if_object_type_does_not_match()
    {
        $arg = new Argument('string');
        $this->assertFalse($arg->match(new stdClass));
        $this->assertFalse($arg->match(new ConsoleLogger));

        $arg2 = new Argument(TestClass::class);
        $this->assertFalse($arg2->match(new ConsoleLogger));
    }

    
    public function test_argument_match_return_true_if_object_type_does_match()
    {
        $arg = new Argument('object');
        $this->assertTrue($arg->match(new stdClass));
        $this->assertTrue($arg->match(new ConsoleLogger));

        $arg2 = new Argument(Logger::class);
        $this->assertTrue($arg2->match(new ConsoleLogger));
    }

    public function test_argument_match_return_true_if_argument_is_optional_and_provided_value_equals_null()
    {
        $arg = new Argument('string', true);
